...
template <size_t Dim>
bool compare_coordinates(const real_coordinate_template<Dim>& a,
                         const real_coordinate_template<Dim>& b) {
...
TEST(BoundingBoxTest, ConvertIntegerGridToReal) {
    // this will test the conversion from a grid coordinate to a real valued coordinate inside the
    // root bounds, not implemented yet
    static constexpr size_t Dim = 3;
